...
- Accept job listing or recruiter/company details
- Generate a short, personalized cold email
- Apply proven cold email structure
- Send or draft the email using Gmail or another email API
- Keep proof of generated and sent outreach emails
...
- A status showing whether the email was drafted or sent
- A log entry for proof/debugging
...
- **Human review required**: Emails must be previewed before sending.
- **Low-volume sending**: The demo should send only a few emails.
- **Personalization required**: The app should avoid sending completely generic emails.
- **No deceptive identity**: The sender must use their own name and email.
- **No fake claims**: The email should not invent experience, referrals, or relationships.
- **Respect opt-outs**: If someone asks not to be contacted again, they should be removed from future outreach.
...
- SendGrid
- Resend
- Groq API (Llama-3/Gemma-2) for high-speed email rewriting
...
- **Step 1: Create Sample Data**: Create 3 sample job/contact records.
- **Step 2: Build the Email Generator**: Write a function that accepts one contact record and returns Subject and Body.
- **Step 3: Add Preview**: Print the generated email clearly in the terminal.
- **Step 4: Add Confirmation**: Ask the user whether to send, skip, or save.
- **Step 5: Add Email Sending**: Use SMTP, Gmail API, or another provider to send the email. For live teaching, start with `DRY_RUN=true`.
- **Step 6: Add Logging**: Write each result to `outreach_log.csv`.
- **Step 7: Turn Off Dry Run**: Send one test email to your own address first.
- **Step 8: Show Proof**: Show the email in the Sent folder or Drafts folder.
...
- Notes on which sending method was used

---

### 19. Micro Skill Badge
- **Badge name**: The Outreach Operator
- **Badge proof**: Screenshot of 3 sent personalized emails from the student’s own address, posted publicly.
